...
public class TileEntityTerrainScanner extends BaseTE implements ITickable {
    ...
    public void update() {
        ...
        if (y < 0 || container.getEnergyStored() < Config.energyPerBlockTerrainScanner) {
            on = false;
            return;
        }
      ...
      //  if(chunk == null)
        ...
        if(worldObj.getBlockState(new BlockPos(x + 1, y, z)).getBlock().isReplaceable(worldObj, new BlockPos(x + 1, y, z)) ||
                worldObj.getBlockState(new BlockPos(x + 1, y, z)).getBlock() instanceof BlockAir) {
            worldObj.setBlockState(new BlockPos(x, y, z), block, 2);
        ...
        }// else System.out.println("im at" + pos + "and" + (x - pos.getX()));
        if (z >= pos.getZ() + 16) {
            y++;
            z = pos.getZ();
        }
        ...
        if(Config.genVanillaOres) {
            if (worldObj.getBlockState(new BlockPos(x, y, z)).getBlock() != Blocks.STONE)
                return;
            if (y > 8) {
                int i = ThreadLocalRandom.current().nextInt(25);
                if (i == 0)
                    worldObj.setBlockState(new BlockPos(x, y, z), Blocks.COAL_ORE.getDefaultState(), 2);
                else if (i == 1)
                    worldObj.setBlockState(new BlockPos(x, y, z), Blocks.IRON_ORE.getDefaultState(), 2);
            }
            if (y > 8 && y < 16) {
                int i = ThreadLocalRandom.current().nextInt(150);
                if (i == 0)
                    worldObj.setBlockState(new BlockPos(x, y, z), Blocks.DIAMOND_ORE.getDefaultState(), 2);
                else if (i == 1)
                    worldObj.setBlockState(new BlockPos(x, y, z), Blocks.EMERALD_ORE.getDefaultState(), 2);
                else if (i == 2)
                    worldObj.setBlockState(new BlockPos(x, y, z), Blocks.REDSTONE_ORE.getDefaultState(), 2);
                else if (i == 3)
                    worldObj.setBlockState(new BlockPos(x, y, z), Blocks.LAPIS_ORE.getDefaultState(), 2);
            }
            if (y > 8 && y < 32) {
                int i = ThreadLocalRandom.current().nextInt(45);
                if (i == 0)
                    worldObj.setBlockState(new BlockPos(x, y, z), Blocks.GOLD_ORE.getDefaultState(), 2);
            }
        }
        Oregistry.getEntryList().stream().filter(entry -> y < entry.maxY && y > entry.minY).forEach(entry -> {
            ...
            if (i == 0) worldObj.setBlockState(new BlockPos(x, y, z), entry.ore, 2);
        });
